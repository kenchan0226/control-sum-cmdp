...
class QA_Metric:
    ...
    def compute_batch(self, questions, true_asws, evaluated_text):
        """
        Calculate the QA scores for a given text we want to evaluate and a list of questions and their answers.
        Args:
          questions: a list of string
        ...
        asw_pred_list, prob_tensor = self.model.predict_batch(questions, evaluated_text)
        # prob_tensor: [batch]

        score_prob_sum = prob_tensor.sum().item()

        score_f_sum = 0.0
        for asw_pred, true_asw in zip(asw_pred_list, true_asws):
            score_f_sum += f1_score(asw_pred, true_asw)

        return {"avg_prob": score_prob_sum / len(questions), "avg_fscore": score_f_sum / len(questions)}
...
def evaluate_corpus_batch(srcs, gens, model=None, questionss=None, aswss=None):
    """
    Calculate the QA scores for an entire corpus.
    Args:
      srcs: a list of string (one string per document)
    ...
    for i, (src, gen) in enumerate(zip(srcs, gens)):
        ...
        # compute the metric
        gen_score = qa_metric.compute_batch(
            masked_questions, masked_question_asws, gen)
        global_score['avg_prob'] += gen_score['avg_prob']
        global_score['avg_fscore'] += gen_score['avg_fscore']
