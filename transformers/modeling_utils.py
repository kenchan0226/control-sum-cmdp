...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def generate(
        ...
        if num_return_sequences != 1:
            ...
            if position_ids is not None:
                position_ids = position_ids.unsqueeze(1).expand(batch_size, num_return_sequences, cur_len)
                position_ids = position_ids.contiguous().view(
                    batch_size * num_return_sequences, cur_len
                )  # (batch_size * num_return_sequences, cur_len)
            if attn_mask is not None:
                attn_mask = attn_mask.unsqueeze(1).expand(batch_size, num_return_sequences, cur_len)
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if position_ids is not None:
                next_position_ids = position_ids[:, -1] + 1  # [batch_size]
                #position_ids = torch.cat([position_ids, next_position_ids.unsqueeze(1)], dim=-1)
                position_ids = next_position_ids.unsqueeze(1)
            if attn_mask is not None:
                next_attn_mask = torch.FloatTensor([1] * attn_mask.size(0)).to(attn_mask.device)  # [batch_size]
                attn_mask = torch.cat([attn_mask, next_attn_mask.unsqueeze(1)], dim=-1)
    ...
    def _generate_beam_search(
        ...
        if position_ids is not None:
            position_ids = position_ids.unsqueeze(1).expand(batch_size, num_beams, cur_len)
            position_ids = position_ids.contiguous().view(batch_size * num_beams, cur_len)  # (batch_size * num_beams, cur_len)
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).expand(batch_size, num_beams, cur_len)
            attn_mask = attn_mask.contiguous().view(batch_size * num_beams, cur_len)  # (batch_size * num_beams, cur_len)
        ...
        while cur_len < max_length:
            ...
            if position_ids is not None:
                position_ids = position_ids[beam_idx, :]  # reorder
                next_position_ids = position_ids[:, -1] + 1  # increment the last positions
                #position_ids = torch.cat([position_ids, next_position_ids.unsqueeze(1)], dim=-1)
                position_ids = next_position_ids.unsqueeze(1)
            if attn_mask is not None:
                attn_mask = attn_mask[beam_idx, :]  # reorder
                next_attn_mask = torch.FloatTensor([1] * attn_mask.size(0)).to(attn_mask.device)  # [batch_size]
                attn_mask = torch.cat([attn_mask, next_attn_mask.unsqueeze(1)], dim=-1)
