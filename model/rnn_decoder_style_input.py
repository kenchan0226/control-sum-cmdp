...
class RNNDecoderStyleInput(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
                 review_attn, pad_idx, attn_mode, dropout=0.0, light_weight=False):
        super(RNNDecoderStyleInput, self).__init__()
    ...
    def forward(self, y, h, memory_bank, src_mask, max_num_oovs, src_oov, coverage, decoder_memory_bank=None, style_embedding=None):
        """
        :param y: [batch_size]
        ...
        :param style_embedding: [batch_size, embed_size]
        :return:
        """
        ...
        if len(style_embedding.size()) == 1:
            style_embedding = style_embedding.unsqueeze(0)

        try:
            rnn_input = torch.cat((y_emb, style_embedding.unsqueeze(0)), dim=2)  # [1, batch_size, embed_size + embed_size]
        except:
            print(y_emb.size())
            print(style_embedding.size())
            exit()
        """
