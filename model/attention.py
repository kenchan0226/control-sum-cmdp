...
class DiversityAttention(nn.Module):
    def __init__(self, decoder_size, memory_bank_size, coverage_attn, use_orthogonal=False):
        super(DiversityAttention, self).__init__()
        # attention
        self.v = nn.Linear(decoder_size, 1, bias=False)
        self.decode_project = nn.Linear(decoder_size, decoder_size)
        self.memory_project = nn.Linear(memory_bank_size, decoder_size, bias=False)
        self.query_ctx_project = nn.Linear(memory_bank_size, decoder_size, bias=False)
        self.coverage_attn = coverage_attn
        if coverage_attn:
            self.coverage_project = nn.Linear(1, decoder_size, bias=False)
        self.softmax = MaskedSoftmax(dim=1)
    ...
    def score(self, memory_bank, decoder_state, query_ctx, coverage=None):
        """
        :param memory_bank: [batch_size, max_input_seq_len, self.num_directions * self.encoder_size]
        :param decoder_state: [batch_size, decoder_size]
        :param query_ctx: [batch_size, self.num_directions * self.encoder_size]
        :param coverage: [batch_size, max_input_seq_len]
        :return: score: [batch_size, max_input_seq_len]
        """
        batch_size, max_input_seq_len, memory_bank_size = list(memory_bank.size())
        ...
        encoder_feature = self.memory_project(memory_bank_)  # [batch_size*max_input_seq_len, decoder size]
        # project query
        query_feature = self.query_ctx_project(query_ctx)  # [batch, decoder_size]
        query_feature_expanded = query_feature.unsqueeze(1).expand(batch_size, max_input_seq_len,
                                                               decoder_size).contiguous()
        query_feature_expanded = query_feature_expanded.view(-1, decoder_size ) # [batch_size*max_input_seq_len, decoder_size]
        # project decoder state
        dec_feature = self.decode_project(decoder_state)  # [batch_size, decoder_size]
        dec_feature_expanded = dec_feature.unsqueeze(1).expand(batch_size, max_input_seq_len,
                                                               decoder_size).contiguous()
        dec_feature_expanded = dec_feature_expanded.view(-1,
                                                         decoder_size)  # [batch_size*max_input_seq_len, decoder_size]
        # sum up attention features
        att_features = encoder_feature + dec_feature_expanded + query_feature_expanded  # [batch_size*max_input_seq_len, decoder_size]
    ...
    def forward(self, decoder_state, memory_bank, query_ctx, src_mask=None, coverage=None):
        """
        :param decoder_state: [batch_size, decoder_size]
        :param memory_bank: [batch_size, max_input_seq_len, self.num_directions * self.encoder_size]
        :param query_ctx: [batch_size, self.num_directions * self.encoder_size]
        ...
        scores = self.score(memory_bank, decoder_state, query_ctx, coverage)  # [batch_size, max_input_seq_len]
        attn_dist = self.softmax(scores, mask=src_mask)  # src_mask: [batch_size, max_input_seq_len]
...
if __name__=='__main__':
    ...
    attention_layer = DiversityAttention(decoder_size, memory_bank_size, coverage_attn)
    max_input_seq_len = 7
    ...
    query_ctx = torch.randn((batch_size, memory_bank_size))
    context, attn_dist, coverage = attention_layer(decoder_state, memory_bank, query_ctx, enc_padding_mask, coverage)

    print("context size:")
    print(context.size())
    print("attn_dist_size:")
