...
from model.attention import Attention, DiversityAttention
import numpy as np
from utils.masked_softmax import MaskedSoftmax
from model.ortho_LSTM import OrthoLSTMCell


class RNNDiversityAttnDecoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
                 review_attn, pad_idx, attn_mode, dropout=0.0, light_weight=False, use_ortho_LSTM=False):
        super(RNNDiversityAttnDecoder, self).__init__()
        ...
        self.attention_layer = DiversityAttention(
            decoder_size=hidden_size,
            memory_bank_size=memory_bank_size,
            coverage_attn=coverage_attn
        )
        self.query_attention_layer = Attention(
            decoder_size=hidden_size,
            memory_bank_size=memory_bank_size,
            coverage_attn=False,
            attn_mode=attn_mode
        ...
        self.use_ortho_LSTM = use_ortho_LSTM
        if use_ortho_LSTM:
            self.ortho_LSTM_cell = OrthoLSTMCell(memory_bank_size, memory_bank_size)
            self.init_h_t = nn.Parameter(torch.FloatTensor(memory_bank_size))
            self.init_c_t = nn.Parameter(torch.FloatTensor(memory_bank_size))
            nn.init.uniform_(self.init_h_t, -0.1, 0.1)
            nn.init.uniform_(self.init_c_t, -0.1, 0.1)
        ...
        if light_weight:
            self.vocab_dist_linear_2 = nn.Linear(hidden_size, self.embed_size)
            self.tanh = nn.Tanh()
        else:
            self.vocab_dist_linear_2 = nn.Linear(hidden_size, vocab_size)
        self.softmax = MaskedSoftmax(dim=1)

    def forward(self, y, h, memory_bank, src_mask, query_memory_bank, query_src_mask, max_num_oovs, src_oov, previous_attn_ctx, coverage, ortho_lstm_h=None, ortho_lstm_c=None, decoder_memory_bank=None):
        """
        :param y: [batch_size]
        ...
        :param previous_attn_ctx: [batch_size, memory_bank_size]
        ...
        rnn_input = torch.cat([y_emb, previous_attn_ctx.unsqueeze(0)], dim=2)  # [1, batch_size, embed_size+memory_bank_size]
        ...
        # attend query
        query_attn_ctx, _, _ = self.query_attention_layer(last_layer_h_next, query_memory_bank, query_src_mask, coverage=None)
        """
        # debug
        print("query_attn_ctx_size")
        print(query_attn_ctx.size())
        """
        # attend document
        context, attn_dist, coverage = self.attention_layer(last_layer_h_next, memory_bank, query_attn_ctx, src_mask, coverage)

        if self.use_ortho_LSTM:
            ortho_lstm_h_next, ortho_lstm_c_next = self.ortho_LSTM_cell(context, ortho_lstm_h, ortho_lstm_c)
            context = ortho_lstm_h_next
        else:
            ortho_lstm_h_next = None
            ortho_lstm_c_next = None

        # debug
        #print("ortho_lstm_h_next")
        #print(ortho_lstm_h_next)
        #print("ortho_lstm_c_next")
        #print(ortho_lstm_c_next)
        #print("context")
        #print(context)
        #print()
        ...
        if self.light_weight:
            dec_out = self.vocab_dist_linear_2(self.tanh(self.vocab_dist_linear_1(vocab_dist_input)))
            vocab_dist = self.softmax(torch.mm(dec_out, self.embedding.weight.t()))
        else:
        ...
        return final_dist, h_next, context, attn_dist, p_gen, coverage, ortho_lstm_h_next, ortho_lstm_c_next
...
if __name__ == '__main__':
    ...
    review_attn = True
    attn_mode = "concat"
    decoder = RNNDiversityAttnDecoder(vocab_size, embed_size, decoder_size, num_layers, memory_bank_size, coverage_attn, copy_attn, review_attn, pad_idx, attn_mode)
    batch_size = 5
    max_src_seq_len = 7
    ...
    query_seq_len = 4
    query_memory_bank = torch.randn((batch_size, query_seq_len, memory_bank_size))
    query_src_mask = torch.ones((batch_size, query_seq_len))
    previous_attn_ctx = torch.randn((batch_size, memory_bank_size))

    final_dist, h_next, context, attn_dist, p_gen, coverage = decoder(y, h, memory_bank, src_mask, query_memory_bank, query_src_mask, max_num_oovs, input_seq_oov, previous_attn_ctx, coverage, trg_side_memory_bank)
    # y, h, memory_bank, src_mask, query_memory_bank, query_src_mask, max_num_oovs, src_oov, previous_attn_ctx, coverage, decoder_memory_bank=None
