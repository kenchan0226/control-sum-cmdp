...
from model.rnn_decoder_style_input import RNNDecoderStyleInput
from utils import io


class Seq2SeqModelExactLenInput(nn.Module):
    """Container module with an encoder, deocder, embeddings."""

    def __init__(self, opt):
        """Initialize model."""
        super(Seq2SeqModelExactLenInput, self).__init__()
        ...
        self.unk_idx = io.UNK
        self.sep_idx = None
        self.len_idx = opt.word2idx[io.EXACT_LEN_WORD]
        ...
        self.decoder = RNNDecoderStyleInput(
            vocab_size=self.vocab_size,
            embed_size=self.emb_dim,
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, exact_output_lens):
        """
        ...
        exact_len_embedding = self.decoder.embedding(len_idx_tensor)  # [batch_size, embed_size]
        # multiplied by length
        exact_output_lens_tensor = torch.FloatTensor(exact_output_lens).to(device=y_t_init.device)  # [batch_size]

        multiplied_exact_len_embedding = torch.bmm(exact_len_embedding.unsqueeze(2), exact_output_lens_tensor.view(-1, 1, 1))
        multiplied_exact_len_embedding = multiplied_exact_len_embedding.squeeze(2)  # [batch_size, embed_size]
        #if len(multiplied_exact_len_embedding.size()) == 1:
        #    multiplied_exact_len_embedding = multiplied_exact_len_embedding.unsqueeze(0)
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_bank, src_mask, max_num_oov, src_oov, coverage, decoder_memory_bank, multiplied_exact_len_embedding)
